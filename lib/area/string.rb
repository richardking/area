class String
  ...
  def to_area
    Rollbar.warning("[area_gem] calling String#to_area") if defined?(Rollbar)
  ...
  def to_region(options = {})
    Rollbar.warning("[area_gem] calling String#to_region") if defined?(Rollbar)
  ...
  def to_zip
    Rollbar.warning("[area_gem] calling String#to_zip") if defined?(Rollbar)
    if self.match(',')
      area = self.split(',')
  ...
  def to_gmt_offset
    Rollbar.warning("[area_gem] calling String#to_gmt_offset") if defined?(Rollbar)
  ...
  def to_dst
    Rollbar.warning("[area_gem] calling String#to_dst") if defined?(Rollbar)
    if Area.zip_or_territory?(self.to_s)
      row = Area.zip_codes.find {|row| row[2] != nil and (row[2].upcase == self.to_s.upcase or row[0] == self.to_s) }
      row[6] if row
    end
  ...
  def observes_dst?
    Rollbar.warning("[area_gem] calling String#observes_dst?") if defined?(Rollbar)
    to_dst == "1"
  end
  ...
  def to_latlon
    Rollbar.warning("[area_gem] calling String#to_latlon") if defined?(Rollbar)
    if Area.zip?(self)
  ...
  def to_lat
    Rollbar.warning("[area_gem] calling String#to_lat") if defined?(Rollbar)
    if Area.zip?(self)
      row = Area.zip_codes.find {|row| row.first == self.to_s }
      row[3] if row
    end
  ...
  def to_lon
    Rollbar.warning("[area_gem] calling String#to_lon") if defined?(Rollbar)
    if Area.zip?(self)
      row = Area.zip_codes.find {|row| row.first == self.to_s }
      row[4] if row
    end
