class Array
  ...
  def to_region(options = {})
    Rollbar.warning("[area_gem] calling Array#to_region") if defined?(Rollbar)
  ...
  def to_zip
    Rollbar.warning("[area_gem] calling Array#to_zip") if defined?(Rollbar)
  ...
  def to_gmt_offset
    Rollbar.warning("[area_gem] calling Array#to_gmt_offset") if defined?(Rollbar)
  ...
  def to_dst
    Rollbar.warning("[area_gem] calling Array#to_dst") if defined?(Rollbar)
    row = Area.zip_codes.find {|row| row[3] == self[0].to_s and row[4] == self[1].to_s }
    row[6] if row
  end
  ...
  def observes_dst?
    Rollbar.warning("[area_gem] calling Array#observes_dst") if defined?(Rollbar)
    to_dst == "1"
  end
